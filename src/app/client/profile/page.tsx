...
import { Paper, Stack, Typography, TextField, Button, Divider, Tabs, Tab, IconButton } from "@mui/material";
...
const UserProfile = () => {
    ...
    const regenerateApiKey = async (e: React.MouseEvent<HTMLButtonElement>) => {
        try {
            const apikey = await authService.regenerateApiKey()
            setUserData({ apiKey: apikey })
        }
        catch {
            toast.error("Error al generar apikey");
        }
    }
    ...
    return (
        <Paper sx={{ p: 4, maxWidth: 600, mx: "auto", my: 4 }}>
            ...
            <Tabs value={tab} onChange={handleTabChange} variant="fullWidth">
                <Tab label="Modificar Usuario" />
                <Tab label="Actualizar Domicilio" />
                <Tab label="Regenerar API Key" />
                <Tab label="Eliminar Cuenta" />
                <Tab label="Informacion de Usuario"></Tab>
            </Tabs>
            ...
            {tab === 2 && (
                <Stack spacing={2} className="text-center">
                    <Typography variant="h6">Regenerar API Key</Typography>
                    ...
                    <Stack direction="row" alignItems="center" spacing={1}>
                        ...
                        >
                            {showApiKey ? userData.apiKey || "No disponible" : "••••••••••••••"}
                        </Typography>
                    ...
                    <Button variant="contained" color="secondary" onClick={regenerateApiKey}>
                        Regenerar API Key
                    </Button>
                </Stack>
            )}
