...
export default function ShipmentRegisterPage() {
	...
	useEffect(() => {
		LocalidadesService.getLocalidades().then((localidades: Localidad[]) => {
			...
			<form className='bg-white shadow-lg rounded-b-md space-y-5 px-8 py-6 flex flex-col gap-4'>
				...
				<FormControl>
					...
					<Input
						id='numero'
						type='number'
						defaultValue={shipment.destino.numero === 0 ? "" : shipment.destino.numero}
				...
				<FormControl>
					<Stack spacing={2} sx={{ width: 300 }}>
						<Autocomplete
							freeSolo
							id={String(shipment.destino.localidadID)}
							disableClearable
