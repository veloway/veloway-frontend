...
export default function DriverHomePage({ params }: driverPageProp) {
	...
	const [allViajesByConductor, setAllViajesByConductor] = useState<GetAllByConductorIDDto[] | null>(null);
	...
	useEffect(() => {
	  ...
	  useEffect(() =>{
		...
		ViajesService.getAllViajesByConductorId(idConductor)
			.then((data) => {
				...
				if(!data) {
					setAllViajesByConductor(null)
				} else {
					setAllViajesByConductor(data)
				}
			})
			.catch((error) => {
				console.error(error);
	  ...
	  if (!allViajesByConductor) return

	  return(
		<div className="min-h-screen bg-gray-100 p-6 mt-20">
			<header className="bg-[#1565c0] text-white rounded-lg shadow-lg p-6 mb-6">
			...
			{ loading ? (
				  ...
				  <div className="p-6 space-y-4">
					<div className="flex items-center space-x-4">
					  ...
					  <div>
						...
						<div className="h-4 w-36 bg-gray-400 rounded mt-2"></div>
					  </div>
					</div>
					<div className="flex items-center space-x-4">
					  <div className="h-10 w-10 bg-gray-400 rounded-full"></div>
					  <div>
						<div className="h-4 w-24 bg-gray-400 rounded"></div>
						<div className="h-4 w-40 bg-gray-400 rounded mt-2"></div>
					  </div>
					</div>
					<div className="flex items-center space-x-4">
					  <div className="h-10 w-10 bg-gray-400 rounded-full"></div>
					...
					<div className="h-10 bg-gray-400 rounded w-36 mx-auto mt-4"></div>
				  </div>
				</div>
			  </div>
			) : viajeActual ? (
					...
					<div className="bg-white rounded-lg shadow-lg overflow-hidden">
						...
						<div className="p-6 space-y-4">
							...
							<div>
								...
								<p className="text-gray-600">{viajeActual?.envio.cliente.nombre} {viajeActual?.envio.cliente.apellido}</p>
							</div>
							</div>
							<div className="flex items-center space-x-4">
							<svg
				...
				<ReservationsTable columns={columnsReservations}/>
			</div>
			) : (
				<SinViaje />
			)}
				...
				<div className="overflow-x-auto">
					...
					<tbody className="bg-white divide-y divide-gray-200">
						...
						<tr key={viaje.idViaje} className="hover:bg-gray-50">
							<td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">{viaje.envio.cliente.nombre} {viaje.envio.cliente.apellido}</td>
							<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{viaje.fechaInicio}</td>
							<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{viaje.envio.origen.calle} {viaje.envio.origen.numero} {viaje.envio.origen.localidad.nombre} {viaje.envio.origen.localidad.provincia.nombre}</td>
							<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{viaje.envio.destino.calle} {viaje.envio.destino.numero} {viaje.envio.destino.localidad.nombre} {viaje.envio.destino.localidad.provincia.nombre}</td>
							<td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{viaje.envio.monto}</td>
						</tr>
						))}
					</tbody>
					</table>
